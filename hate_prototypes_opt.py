...
@torch.no_grad()
def collect_cls_embeddings(model, loader, device):
    """Extract last-layer [CLS] representations."""
    model.eval()
...
def parse_args():
    ...
    parser.add_argument(
        "--out_dir",
        default="predictions-bert-full-protos",
    )
...
def main():
    ...
    for source in args.datasets:
        ...
        for seed in args.seeds:
            ...
            model_name = args.model_pattern.format(
                ds=source,
                seed=seed,
            )
            ...
            # Extract test representations once per encoder.
            test_cache = {}
            ...
            for target in args.datasets:
                ...
                test_cache[target] = collect_cls_embeddings(
                    model,
                    loader,
                    device,
                )

            # Construct prototypes from every dataset.
            prototypes = {}
            ...
            for prototype_domain in args.datasets:
                ...
                features, labels = collect_cls_embeddings(
                    model,
                    loader,
                    device,
                )
            ...
            # Prototype domain P -> evaluation domain T.
            for prototype_domain in args.datasets:
